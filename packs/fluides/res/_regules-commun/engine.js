(function () {
  ...
  function renderFilm() {
    ...
    article.innerHTML = '<section class="film-intro"><p class="kicker">1 · Commencer par le film</p><h2>' + escapeHtml(module.title) + '</h2><p class="lead">Regarde d’abord le fonctionnement complet : circuit fluidique, commande électrique et actions restent visibles ensemble.</p>' + choices + '<p class="film-audio-note">' + (hasNarration ? 'La voix du film se lance uniquement avec le bouton « Écouter les explications ».' : 'Ce film est visuel. La voix du cours est disponible à l’écran suivant.') + '</p><a class="film-open" href="../_regules-commun/films/' + escapeHtml(film.fichier) + '?v=' + encodeURIComponent(catalog.version || "") + '" target="_blank" rel="noopener">Ouvrir le film en grand ↗</a></section><section class="film-stage"><iframe id="film-frame" src="../_regules-commun/films/' + escapeHtml(film.fichier) + '?v=' + encodeURIComponent(catalog.version || "") + '" title="Film : ' + escapeHtml(film.titre) + '" loading="eager"></iframe></section>';
