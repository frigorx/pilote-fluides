(() => {
  ...
  const buildScene = svg("bi-build", "Connexions fonctionnelles d’un réseau bitube", "Chaque radiateur possède une entrée raccordée au départ et une sortie raccordée au retour. Une branche qui rejoint départ-départ ou retour-retour n’est pas fonctionnelle dans ce modèle.", `
    ...
    ${[210,380,550].map((x,i)=>`<g><rect x="${x-75}" y="166" width="150" height="92" rx="15" fill="#fffdf8" stroke="#1b3a63" stroke-width="4"/><text x="${x}" y="205" text-anchor="middle" font-size="20" font-weight="700">ÉMETTEUR ${i+1}</text><text x="${x}" y="231" text-anchor="middle" font-size="20">entrée/sortie</text><path d="M${x} 85V166M${x} 258V345" stroke="#1b3a63" stroke-width="7" stroke-dasharray="10 7"/></g>`).join("")}
    <text x="115" y="61" font-size="20" font-weight="700">DÉPART COMMUN</text><text x="115" y="382" font-size="20" font-weight="700">RETOUR COMMUN</text>`);
