(() => {
  ...
  window.STATION_CONFIG = {
    ...
    steps: [
      ...
      { short: "Vérifier", narration: "Terminons par la méthode, parce que c'est elle qui vous évitera de perdre une journée. Avant d'équilibrer, on compare. Un déséquilibre supposé d'après la géométrie du réseau n'est pas un déséquilibre constaté : il faut des débits relevés, branche par branche, dans un état stabilisé. Confondre ce que le plan laisse prévoir et ce que l'installation fait réellement, c'est régler à l'aveugle. La bonne séquence est toujours la même : je relève, je constate, j'agis sur une seule chose, j'attends, je relève à nouveau.", kicker: "méthode", title: "Comparer avant d’équilibrer", text: "Ordonnez une démarche qui ne confond pas géométrie et perte réelle.", cap: "Relevez l’état de chaque branche avant de comparer.", tp: "Relevez l’état et comparez les branches.", bts: "Calculez au débit visé, puis confrontez aux mesures.", scene: methodScene, equivalent: "Tracer, inventorier, calculer, mesurer puis équilibrer.", action: { type: "sequence", prompt: "Ordonnez la comparaison.", items: ["Mesurer dans un état connu","Tracer chaque chemin","Équilibrer et contrôler","Inventorier diamètres et singularités","Calculer au débit visé"], correctOrder: [1,3,4,0,2], explain: "Le réglage vient après la compréhension et la mesure du réseau." } }
    ],
    quiz: [
