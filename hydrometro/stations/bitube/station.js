  ...
  function renderRange(action, step) {
    ...
    els.actionPanel.innerHTML = `
      ...
      <div class="range-row">
        ...
        <div class="range-control"><button type="button" class="range-step" data-range-step="-1" aria-label="Diminuer ${escapeHtml(action.label)}">−</button><input id="stationRange" type="range" min="${action.min}" max="${action.max}" step="${action.step}" value="${initial}"><button type="button" class="range-step" data-range-step="1" aria-label="Augmenter ${escapeHtml(action.label)}">+</button></div>
      </div>`;
    ...
    els.actionPanel.querySelectorAll("[data-range-step]").forEach((button) => button.addEventListener("click", () => {
      const next = Math.max(action.min, Math.min(action.max, Number(input.value) + Number(button.dataset.rangeStep) * action.step));
