(() => {
  ...
  const symbol = shell("circ-symbol", "Circulateur et sens d’écoulement",
    ...
    `<path d="M55 220H260M460 220H665" stroke="#1b3a63" stroke-width="15"/>
     ...
     <text x="545" y="133" text-anchor="middle" font-size="19" font-weight="700">ÉCOULEMENT →</text>
     <rect x="90" y="44" width="540" height="60" rx="18" fill="#fff4e0" stroke="#b06a00" stroke-width="3" stroke-dasharray="5 5"/>
     <text x="360" y="70" text-anchor="middle" font-size="19" font-weight="700">DÉBIT Q · m³/h</text><text x="360" y="94" text-anchor="middle" font-size="19">HAUTEUR H · mCE</text>
     <text x="360" y="365" text-anchor="middle" font-size="19">Dans une boucle fermée, H sert surtout à vaincre les pertes de charge.</text>`);
  ...
  function curveScene({ k = .5, speed = 1, id = "circ-curve" } = {}) {
    ...
    const hOp = netH(qOp);
    const px = x(qOp), py = y(Math.min(6, hOp));
    const lx = 520, ly = 312;
    const curve = (fn) => {
      const points = [];
    ...
    return shell(id, "Courbes pompe et réseau", desc,
      `<rect x="55" y="42" width="630" height="335" rx="18" fill="#fffdf8" stroke="#1b3a63" stroke-width="3"/>
       ${Array.from({length:6},(_,i)=>`<path d="M80 ${y(i)}H650" stroke="rgba(27,58,99,.13)" stroke-width="1"/><text x="70" y="${y(i)+5}" text-anchor="end" font-size="19">${i}</text>`).join("")}
       ${Array.from({length:6},(_,i)=>`<path d="M${x(i)} 350V74" stroke="rgba(27,58,99,.13)" stroke-width="1"/><text x="${x(i)}" y="369" text-anchor="middle" font-size="19">${i}</text>`).join("")}
       ...
       <circle cx="${px}" cy="${py}" r="11" fill="#fffdf8" stroke="#1e7e54" stroke-width="6"/>
       <path d="M${px} ${py}L${lx} ${ly + 14}" stroke="#1e7e54" stroke-width="2" stroke-dasharray="3 4"/>
       <rect x="${lx - 116}" y="${ly - 9}" width="232" height="30" rx="8" fill="#fffdf8" stroke="#1e7e54" stroke-width="2"/>
       <text x="${lx}" y="${ly + 12}" text-anchor="middle" font-size="19" font-weight="700">POINT DE FONCTIONNEMENT</text>
       <text x="365" y="405" text-anchor="middle" font-size="19" font-weight="700">DÉBIT Q (m³/h)</text><text x="20" y="220" transform="rotate(-90 20 220)" text-anchor="middle" font-size="19" font-weight="700">HAUTEUR H (mCE)</text>
       <text x="505" y="65" font-size="19" fill="#3d7fca" style="fill:#3d7fca" font-weight="700">— POMPE</text><text x="585" y="65" font-size="19" fill="#c9451a" style="fill:#c9451a" font-weight="700">-- RÉSEAU</text>
       <rect x="92" y="88" width="166" height="48" rx="12" fill="#e3f5ec" stroke="#1e7e54" stroke-width="4"/><text x="175" y="109" text-anchor="middle" font-size="19" font-weight="700">Q ≈ ${qOp.toFixed(1)} m³/h</text><text x="175" y="129" text-anchor="middle" font-size="19" font-weight="700">H ≈ ${hOp.toFixed(1)} mCE</text>`);
  }
  ...
  const measurements = shell("circ-measure", "Points de mesure autour du circulateur",
    ...
    `<path d="M70 225H650" stroke="#1b3a63" stroke-width="15"/>
     ...
     <rect x="50" y="78" width="150" height="70" rx="14" fill="#fffdf8" stroke="#1b3a63" stroke-width="3"/><text x="125" y="108" text-anchor="middle" font-size="19" font-weight="700">COMMANDE</text><text x="125" y="132" text-anchor="middle" font-size="19">mode / vitesse</text>
     <path d="M218 175V148M502 175V148" stroke="#1b3a63" stroke-width="3" stroke-dasharray="5 4"/>
     <text x="360" y="356" text-anchor="middle" font-size="19" font-weight="700">Δp + Q + COMMANDE + ÉTAT DU RÉSEAU</text>`);
  ...
  const diagnosis = shell("circ-diag", "Démarche de réglage contrôlé",
    ...
    `<g fill="#fffdf8" stroke="#1b3a63" stroke-width="3">${["État initial", "Mesurer", "Régler", "Stabiliser", "Relever", "Conclure"].map((t,i)=>`<rect x="${18+i*117}" y="160" width="103" height="82" rx="14"/><text x="${69+i*117}" y="194" text-anchor="middle" font-size="19" font-weight="700">${t}</text><text x="${69+i*117}" y="218" text-anchor="middle" font-size="19">${i+1}</text>`).join("")}</g>
     ...
     <text x="220" y="79" text-anchor="middle" font-size="19" font-weight="700">AVANT · vitesse 100 %</text>
     <text x="220" y="103" text-anchor="middle" font-size="19">Q = 2,4 m³/h · Δp = 0,42 bar</text>
     <text x="220" y="125" text-anchor="middle" font-size="19">bruit d’écoulement notable</text>
     ...
     <text x="510" y="79" text-anchor="middle" font-size="19" font-weight="700">APRÈS · vitesse 80 %</text>
