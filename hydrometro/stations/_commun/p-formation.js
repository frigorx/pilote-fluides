(() => {
  ...
  const catalogue = {
    ...
    debit: {
      ...
      narrationEssai: "Vous réglez ici la commande de la pompe et l'ouverture du réseau, en observant ce qui se passe sur les deux courbes à droite. Une zone à atteindre apparaît sur l'écran : c'est votre repère. Bougez un seul curseur à la fois, la pompe puis le réseau, en suivant de quel côté se déplace le point où les deux courbes se croisent. Ce déplacement, plus que la valeur finale, est ce qui compte ici : il montre lequel des deux réglages agit dans quel sens. Une fois ce sens compris, ajustez les deux ensemble jusqu'à entrer dans la zone attendue.",
      narrationSynthese: "Vous venez de constater que le débit n'est jamais le choix d'un seul réglage : il naît de la rencontre entre ce que la pompe propose et ce que le réseau impose. Toucher l'un déplace le point, même sans toucher l'autre. Sur une installation réelle, c'est ce réflexe qui vous évite de conclure trop vite qu'une pompe est sous-dimensionnée alors qu'une vanne fermée bride tout le circuit. Ce constat vous servira chaque fois qu'un client se plaint d'un manque de chaleur. La station suivante s'appuie directement sur ce débit pour aller plus loin dans la lecture de l'installation."
    },
    "delta-t": {
  ...
  window.HydroStation.init = (stationConfig) => {
    ...
    style.rel = "stylesheet"; style.href = "../_commun/p-formation.css?v=20260915-0911"; document.head.append(style);
    ...
    function speak() {
      ...
      const dit = current === lessons.length ? (catalogue[id]?.narrationEssai || "").trim()
        : current === lessons.length + 1 ? (catalogue[id]?.narrationSynthese || "").trim()
        : (lessons[current]?.narration || "").trim();
