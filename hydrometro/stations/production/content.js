(() => {
  ...
  const network = (id, active = 1) => shell(id,
    ...
    `<rect x="40" y="76" width="210" height="260" rx="24" fill="#fffdf8" stroke="#1b3a63" stroke-width="4"/>
     ...
     <text x="145" y="305" text-anchor="middle" font-size="19">fonction : échanger</text>
     ...
     <text x="570" y="305" text-anchor="middle" font-size="19" font-weight="700">USAGES</text>
     <text x="300" y="124" text-anchor="middle" font-size="19" font-weight="700">DÉPART</text>
     <text x="405" y="329" text-anchor="middle" font-size="19" font-weight="700">RETOUR</text>
     <g transform="translate(48 34)"><rect width="204" height="30" rx="15" fill="#1b3a63"/><text x="102" y="21" text-anchor="middle" font-size="19" fill="#fff" style="fill:#fff">Bloc actif : ${active}</text></g>`);
  ...
  const technologies = shell("prod-tech", "Fonction commune, technologies différentes",
    ...
    `<text x="360" y="42" text-anchor="middle" font-size="22" font-weight="700">UNE FONCTION · PLUSIEURS TECHNOLOGIES</text>
     ...
     <text x="137" y="122" text-anchor="middle" font-size="19" font-weight="700">Chaudière</text><text x="137" y="144" text-anchor="middle" font-size="19">brûle un combustible</text>
     ...
     <text x="360" y="122" text-anchor="middle" font-size="19" font-weight="700">Pompe à chaleur</text><text x="360" y="144" text-anchor="middle" font-size="19">prend l’énergie dehors</text>
     ...
     <text x="583" y="122" text-anchor="middle" font-size="19" font-weight="700">Groupe d’eau glacée</text><text x="583" y="144" text-anchor="middle" font-size="19">produit du froid</text>
     <path d="M137 220L300 290M360 220v70M583 220L420 290" fill="none" stroke="#3d7fca" stroke-width="7" marker-end="url(#arrow-prod-tech)"/>
     <rect x="250" y="290" width="220" height="112" rx="22" fill="url(#warm-prod-tech)" stroke="#c9451a" stroke-width="4"/>
     <text x="360" y="347" text-anchor="middle" font-size="19" font-weight="700">TRANSFÉRER L’ÉNERGIE</text><text x="360" y="373" text-anchor="middle" font-size="19">fonction hydraulique</text>`);
  ...
  const measurements = shell("prod-measure", "Points de contrôle d’un générateur",
    ...
    `<rect x="250" y="120" width="220" height="180" rx="24" fill="#fffdf8" stroke="#1b3a63" stroke-width="4"/>
     ...
     <path d="M230 210H250" stroke="#1b3a63" stroke-width="5" marker-end="url(#arrow-prod-measure)"/>
     <text x="575" y="110" text-anchor="middle" font-size="19" font-weight="700">DÉPART</text><text x="575" y="321" text-anchor="middle" font-size="19" font-weight="700">RETOUR</text>`);
  ...
  const loadScene = (value = 50) => {
    ...
    return shell("prod-load", "Profil de charge simplifié", `Le besoin relatif vaut ${value} pour cent. ${blocks} bloc ou blocs de production sont représentés comme actifs. Cette scène ne calcule ni rendement ni sélection réelle.`,
      `<text x="360" y="44" text-anchor="middle" font-size="22" font-weight="700">BESOIN RELATIF : ${value} %</text>
       ...
       <g transform="translate(85 205)">${[0,1,2].map((n) => `<rect x="${n*190}" width="160" height="112" rx="18" fill="${n < blocks ? "url(#warm-prod-load)" : "#fffdf8"}" stroke="${n < blocks ? "#c9451a" : "#637285"}" stroke-width="4" stroke-dasharray="${n < blocks ? "0" : "8 6"}"/><text x="${n*190+80}" y="52" text-anchor="middle" font-size="19" font-weight="700">BLOC ${n+1}</text><text x="${n*190+80}" y="82" text-anchor="middle" font-size="19">${n < blocks ? "ACTIF" : "ARRÊT"}</text>`).join("")}</g>
       <text x="360" y="372" text-anchor="middle" font-size="19">Modèle qualitatif : vérifier les données constructeur et le schéma réel.</text>`);
  };
  ...
  const diagnosis = shell("prod-diag", "Démarche avant conclusion",
    ...
    `<g fill="#fffdf8" stroke="#1b3a63" stroke-width="3">${["Dossier", "Mesures", "Comparer", "Hypothèse"].map((label, index) => `<rect x="${28 + index*174}" y="160" width="142" height="92" rx="16"/><text x="${99 + index*174}" y="214" text-anchor="middle" font-size="19" font-weight="700">${label}</text>`).join("")}</g>
     ...
     <rect x="180" y="302" width="360" height="55" rx="16" fill="#fff4e0" stroke="#b06a00" stroke-width="4" stroke-dasharray="4 5"/><text x="360" y="336" text-anchor="middle" font-size="19" font-weight="700">CONCLUSION PROVISOIRE ET JUSTIFIÉE</text>`);
    ...
    steps: [
      {
        ...
        text: "Le nom commercial ne suffit pas. Commencez par la fonction rendue au réseau d’eau.",
