...
(() => {
  ...
  function init(config) {
    ...
    function renderLevel() {
      ...
      if (levelObjective) levelObjective.textContent = current.objective;
    ...
    $("#backPlan").addEventListener("click", () => { window.location.href = "../../index.html#visited=boucle"; });

    function renderQuiz() {
      const q = config.quiz[quizIndex];
      answered = false;
      quizView.innerHTML = `<article class="quiz-card"><div class="quiz-progress">Question ${quizIndex + 1} sur ${config.quiz.length} · après la formation guidée</div><div class="question">${q.prompt}</div><div class="answers" role="group" aria-label="Réponses">${q.options.map((option, index) => `<button type="button" data-answer="${index}">${option}</button>`).join("")}</div><div><p class="explanation" aria-live="polite">Choisis une réponse. La correction expliquera le raisonnement.</p><div class="quiz-actions"><button type="button" data-quiz-action="activity">Revoir la formation</button><button type="button" class="primary" data-quiz-action="next" disabled>${quizIndex === config.quiz.length - 1 ? "Voir le bilan" : "Question suivante"}</button></div></div></article>`;
    ...
    function renderResult() {
      ...
      const state = score >= 3 ? "Objectif atteint" : score === 2 ? "Encore fragile" : "À reprendre";
    ...
    function showActivity() {
      ...
      document.dispatchEvent(new CustomEvent("hydro:return-course"));
    }
    ...
    function startAssessment() {
      ...
      document.dispatchEvent(new CustomEvent("hydro:assessment-start"));
      renderQuiz();
    }
