...
(() => {
  ...
  function init(config) {
    ...
    function renderResult() {
      ...
      resultView.innerHTML = `<article class="result-card"><div class="kicker">Bilan de station</div><h2>${config.title}</h2><p class="result-state ${stateClass}">${state}</p><p><strong>${score} / ${config.quiz.length}</strong> réponses justes.</p><p>${score >= 3 ? config.successMessage : "Refaites la manipulation et lisez les corrections avant un nouvel essai."}</p><p>Ce résultat est un entraînement. Il ne valide aucun diplôme.</p><div class="choice-grid"><button type="button" data-result="retry">Refaire les questions</button><button type="button" class="primary" data-result="next">${config.nextLabel}</button></div></article>`;
