(() => {
  ...
  const orderScene = svg("mono-order", "Amont et aval sur une boucle monotube", "L’émetteur 1 est en amont des émetteurs 2 et 3. Une modification à l’amont peut changer les conditions rencontrées en aval, sans prouver à elle seule une panne.", `
    ...
    ${[180,380,580].map((x, i) => `<g><circle cx="${x}" cy="220" r="52" fill="#fffdf8" stroke="#3d7fca" stroke-width="6"/><text x="${x}" y="227" text-anchor="middle" font-size="20" font-weight="700">Émetteur ${i+1}</text></g>`).join("")}
    <rect x="95" y="95" width="180" height="54" rx="13" fill="#e3f5ec" stroke="#1e7e54" stroke-width="5"/><text x="185" y="128" text-anchor="middle" font-size="20" font-weight="700">AMONT · premier</text>
    <rect x="485" y="95" width="180" height="54" rx="13" fill="#fff4e0" stroke="#b06a00" stroke-width="4" stroke-dasharray="6 5"/><text x="575" y="128" text-anchor="middle" font-size="20" font-weight="700">AVAL · ensuite</text>
    <text x="380" y="360" text-anchor="middle" font-size="20">L’ordre hydraulique est celui du sens de circulation.</text>`);
  ...
  function bypassScene(percent = 35) {
    ...
    return svg("mono-bypass", "Partage du débit dans une dérivation monotube", `Donnée pédagogique : le débit principal vaut 1,00 mètre cube par heure. ${branch.toFixed(2)} passe dans l’émetteur et ${bypass.toFixed(2)} dans le bypass. Avec 60 degrés en amont et 52 degrés en sortie d’émetteur, le mélange aval vaut ${mixed.toFixed(1)} degrés.`, `
      ...
      <rect x="490" y="175" width="205" height="74" rx="14" fill="#fff4e0" stroke="#b06a00" stroke-width="4" stroke-dasharray="6 5"/><text x="592" y="204" text-anchor="middle" font-size="20" font-weight="700">MÉLANGE AVAL</text><text x="592" y="231" text-anchor="middle" font-size="20">1,00 m³/h · ${mixed.toFixed(1)} °C</text>
      <text x="380" y="367" text-anchor="middle" font-size="20">Émetteur : ${branch.toFixed(2)} m³/h · bypass : ${bypass.toFixed(2)} m³/h</text>
      <text x="380" y="397" text-anchor="middle" font-size="20">Hypothèses : eau, pertes de ligne négligées, sortie émetteur fixée à 52 °C.</text>`);
  }
  ...
  const methodScene = svg("mono-method", "Méthode de lecture d’un monotube", "La démarche est posée sur le réseau : suivre le sens sur la conduite, repérer la dérivation et son bypass, attendre l’état stable, relever la température avant l’émetteur soit 60 degrés et après l’émetteur soit 54 degrés dans cet exemple, puis justifier par écrit.", `
    ...
    <circle cx="135" cy="309" r="16" fill="#e3f5ec" stroke="#1e7e54" stroke-width="4"/><text x="135" y="315" text-anchor="middle" font-size="20" font-weight="700">4</text>
    <rect x="160" y="285" width="130" height="48" rx="11" fill="#fffdf8" stroke="#1b3a63" stroke-width="3"/><text x="225" y="315" text-anchor="middle" font-size="20" font-weight="700">AVANT : 60 °C</text>
    <rect x="440" y="285" width="130" height="48" rx="11" fill="#fffdf8" stroke="#1b3a63" stroke-width="3"/><text x="505" y="315" text-anchor="middle" font-size="20" font-weight="700">APRÈS : 54 °C</text>
    <path d="M250 285V218M505 285V218" stroke="#1b3a63" stroke-width="3" stroke-dasharray="5 4"/>
    <text x="365" y="378" text-anchor="middle" font-size="20" font-weight="700">Relever avant / après l’émetteur</text>
    <circle cx="585" cy="309" r="16" fill="#e3f5ec" stroke="#1e7e54" stroke-width="4"/><text x="585" y="315" text-anchor="middle" font-size="20" font-weight="700">5</text>
    <rect x="610" y="275" width="110" height="70" rx="10" fill="#fffdf8" stroke="#1b3a63" stroke-width="3"/>
