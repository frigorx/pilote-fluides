(() => {
  ...
  function renderDualRange(action, step) {
    const initial = Object.fromEntries(action.controls.map((control) => [control.id, control.value]));
    els.actionPanel.innerHTML = `
      <p class="action-title">${escapeHtml(action.prompt)}</p>
      <div class="dual-range">${action.controls.map((control) => `
        <div class="range-row">
          <label for="range-${control.id}">${escapeHtml(control.label)}</label>
          <output id="readout-${control.id}" class="readout"></output>
          <input id="range-${control.id}" data-dual="${control.id}" type="range" min="${control.min}" max="${control.max}" step="${control.step}" value="${control.value}">
        </div>`).join("")}
      </div>`;
    const inputs = [...els.actionPanel.querySelectorAll("[data-dual]")];
    const update = (isUser) => {
      const values = Object.fromEntries(inputs.map((input) => [input.dataset.dual, Number(input.value)]));
      action.controls.forEach((control) => {
        document.getElementById(`readout-${control.id}`).textContent = `${values[control.id].toLocaleString("fr-FR")} ${control.unit}`;
      });
  ...
  function renderLesson() {
    ...
    const sceneValue = step.action?.type === "dual-range" ? Object.fromEntries(step.action.controls.map((control) => [control.id, control.value])) : step.action?.value;
    renderScene(step, sceneValue);
    renderAction(step);
