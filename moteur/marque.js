...
(function () {
  ...
  var css =
    ...
    /* mode papier : rien à l'écran (l'en-tête porte déjà le logo), tout au
       papier — la règle @media print plus bas le remet en pied de feuille. */
    "#marque-inerweb.marque-papier{display:none}" +
  ...
  /* PIÈGE MESURÉ (18/08) : plusieurs modules ont une barre de pilotage fixée
     en bas (bouton « Retour », progression, « Continuer »). Le filigrane, posé
     à 8 px du bas, passait DESSUS — sur le détendeur, il recouvrait le bouton
     Retour et le texte d'étape. On mesure donc ce qui occupe déjà le bas de
     l'écran et on se range au-dessus. Rien à déclarer dans la page : une barre
     `position:fixed` qui touche le bas se détecte toute seule. */
  function hauteurBasOccupe() {
    if (!document.body) return 0;
    var vue = window.innerHeight || document.documentElement.clientHeight;
    var h = 0;
    /* On cherche par GÉOMÉTRIE, pas par `position:fixed` : sur ces modules la
       barre est en flux `static`, au fond d'un conteneur en 100dvh. Ce qui la
       caractérise, c'est qu'elle touche le bas de l'écran, qu'elle est large et
       qu'elle reste basse. Le conteneur pleine hauteur est écarté par le
       plafond à 45 % de la vue. */
    ...
    for (var i = 0; i < noeuds.length; i++) {
      ...
      if (r.height < 8 || r.width < 120) continue;
      if (Math.abs(r.bottom - vue) > 4) continue; // ne touche pas le bas
      if (r.height > vue * 0.45) continue; // c'est un panneau, pas une barre
      if (r.height > h) h = r.height;
    }
    return h;
  }

  /* La page porte-t-elle déjà le logo figé dans son en-tête ? On le reconnaît
     à son étiquette d'accessibilité, la seule marque stable du gabarit. */
  function logoDejaEnEntete() {
    var s = document.querySelector('svg[aria-label^="inerWeb"]');
    return !!(s && !el.contains(s));
  }

  function appliquerMode() {
    if (!el) return;
    var m = modeVoulu();
    /* ARBITRAGE MESURÉ (18/08) — sur les modules à barre de pilotage, le bas
       de l'écran est pris sur toute sa largeur ET sa hauteur : la barre en
       bas, la colonne des étapes à gauche. Remonter le filigrane le faisait
       simplement tomber sur le stepper : neuf recouvrements au lieu de quatre.
       Or ces pages portent maintenant le logo figé dans leur en-tête — le
       filigrane y fait doublon À L'ÉCRAN. Il s'efface donc de l'écran et ne
       reparaît qu'À L'IMPRESSION, seul endroit où il porte encore quelque
       chose que l'en-tête ne dit pas : la licence et le contact. Aucune règle
       de la charte n'est perdue — la marque reste visible sur écran (en-tête)
       comme sur papier (pied de page). */
    if (m === "marque-fixe" && hauteurBasOccupe() > 0 && logoDejaEnEntete()) {
      m = "marque-papier";
    }
    el.style.bottom = "";
