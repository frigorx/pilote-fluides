...
(window.AEROREZO_STATIONS = window.AEROREZO_STATIONS || []).push({
  ...
  cap: "Suivez deux flux qui deviennent un air mélangé.",
  bac: "Calculez une température de mélange simple pour des débits comparables.",
  bts: "Réalisez le bilan massique/énergétique et suivez l’encrassement par pression différentielle.",

  /* Découvrir — ce que l'élève observe, pas ce qu'il doit conclure. */
  decouverte: "Les deux gaines qui entrent n’ont pas la même épaisseur : le trait dit la proportion des débits. D’un côté 700 m³/h d’air extérieur, de l’autre 1 300 m³/h d’air repris. Lisez la température qui sort du caisson, puis faites dans votre tête la moyenne des deux températures d’entrée. Les deux nombres ne sont pas les mêmes — et c’est le trait le plus épais qui explique l’écart.",
  ...
  method: "Pesez chaque température par son débit avant d’additionner. Le résultat tombe toujours entre les deux valeurs d’entrée, et penche du côté du plus gros débit.",
  formula: "T mélange = (Q₁T₁ + Q₂T₂) / (Q₁ + Q₂) — débits en m³/h, températures en °C",

  /* Manipuler — une action précise, avec des valeurs concrètes. */
  consigne: "Partez des valeurs affichées — 700 m³/h d’air extérieur à 5 °C, 1 300 m³/h d’air repris à 22 °C — et notez la température de mélange obtenue. Échangez ensuite les deux débits sans toucher aux températures, et regardez le résultat descendre de plusieurs degrés. Terminez en réglant les deux débits à 1 000 m³/h : vous devez retrouver exactement la moyenne des deux températures.",
  ...
  narration: {
    ...
    comprendre: "Regardez le caisson : deux gaines entrent, une seule ressort. Pourquoi s’imposer ça ? Sans air neuf, humidité et odeurs s’accumulent, et une salle pleine devient irrespirable avant midi. Mais cet air arrive à la température du dehors, glacé en janvier, brûlant en août, et la batterie rattrape tout. L’air repris arrive déjà tempéré : de la chaleur déjà payée. Les registres, ces volets couplés, dosent le partage. Reste la température de sortie. L’intuition trompe : un débit deux fois plus gros, sa température pèse deux fois plus que l’autre. Chacune compte dans l’addition à hauteur de son débit, la somme rapportée au débit total. Tant que les deux débits diffèrent, additionner les deux thermomètres et diviser par deux donne un faux résultat ; à débits identiques seulement, ça retombe juste. Deux réflexes, sans calculatrice. Jamais plus froid que le plus froid des deux, jamais plus chaud que le plus chaud. Et il colle au flux dominant : sur site, un mélange qui frôle la température du dehors trahit un air neuf entré en grand, registre coincé, consigne oubliée, et une batterie qui tire à plein pour rien.",
